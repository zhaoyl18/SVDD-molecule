...
def main(work_type_args):
    ...
    # -------- Generation --------
        ...
        reward_model_preds, selected_baseline_preds, baseline_preds = sampler.controlled_tds_decode(alpha=args.alpha, largerbetter=True)
        ...
        np.savez( "./log/%s-%s-%s-%s-tds" %(config.data.data, args.sample_M, args.reward_name, args.version), decoding = hepg2_values_ours, baseline = hepg2_values_baseline)
